...
- Understand the problem
  ...
  - Rules
    ...
    - input Array size be odd

- Test cases
  - Covered by examples

- Algorithm
  Method `bubble_sort!`, args: `array`
    - loop
      - assign `false` to `swapped`
      - iterate over array with index as `element` and `index`
      - assign (`index` + 1) to `next_index`
      - assign array element at index `next_index` to `next_element`
      - if `next_element` is present
        - if `element` > `next_element`
        - assign `element` to Array at index `next_index`
        - assign `next_element` to Array at index `index`
        - assign `true` to `swapped`
      - else
        - do nothing
